 ...
 * of choice!
 *
 * Code is partially by Maarten Janssen (maarten@cheerful.nl) 2018-04-30
...
SerialStreamFile radFile(Serial); // Serial1 (Bluetooth)
//SerialStreamFile test(Serial1);
...
void loop()
{
  ...
  /* Test Serial Stream:
  test.open("adlibsp.rad"); 
  delay(1000); 
  test.seekSet(17); 
  delay(1000);
  uint16_t data = test.read(); 
  delay(1000);
  uint32_t curPos = test.curPosition(); 
  delay(1000);
 ...
 */
void loadRadFile(const char* fileName) {
  radFile.open(fileName);

  // Set the new position in bytes from the beginning of the file
